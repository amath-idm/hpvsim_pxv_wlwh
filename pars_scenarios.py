...
def get_vx_intvs(routine_start_year=2020, catch_up_year=2025, vx_coverage=0.9, age_range=(9,14), target_PLWH=False):
    ...
    if target_PLWH:
        uptake = 0.8
        px_eligible = lambda sim: (sim.people.hiv) & (sim.t == sim.people.date_screened)
...
def get_screen_intvs(primary=None, triage=None, screen_coverage=0.7, tx_coverage=0.9, ltfu=0.3, start_year=2025):
    '''
    ...
    if triage is not None:
        st_intvs = [screening, screening_hiv, triage_screening, assign_treatment, ablation, excision, radiation]
    else:
        st_intvs = [screening, screening_hiv, triage_screening, ablation, excision, radiation]
